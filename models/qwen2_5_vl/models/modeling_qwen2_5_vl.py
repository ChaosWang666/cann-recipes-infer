...
"""Qwen2.5-VL model definition adapted for Ascend NPUs.

This implementation is derived from the open-source HuggingFace reference but integrates
tensor-parallel aware linear layers and NPU fused operators in the critical computation
paths. The design mirrors the structure used by the Qwen3-MoE recipe so that both models
share the same runtime assumptions and communication helpers.
"""
...
from module.linear import ColumnParallelLinear, RowParallelLinear, VocabParallelEmbedding
...
_warning_once_messages: set[str] = set()


def warning_once(message: str, *args, **kwargs) -> None:
    if message not in _warning_once_messages:
        _warning_once_messages.add(message)
        logger.warning(message, *args, **kwargs)


def _set_weight_loader(module: nn.Module):
    module.weight_loader = default_weight_loader
    return module


def build_column_parallel_linear(
    input_size: int,
    output_size: int,
    *,
    bias: bool = False,
    ...
    return _set_weight_loader(linear)


class Qwen2_5_VLParallelContext:
    def __init__(self, runner_settings: Optional[Dict[str, Any]] = None):
        runner_settings = runner_settings or {}
        self.runner_settings = runner_settings
        parallel_config = runner_settings.get("parallel_config", {})
        self.embed_tp_size = parallel_config.get("embed_tp_size", 1)
        self.attn_tp_size = parallel_config.get("attn_tp_size", 1)
        self.lmhead_tp_size = parallel_config.get("lmhead_tp_size", self.embed_tp_size)
        self.world_size = runner_settings.get("world_size", dist.get_world_size() if dist.is_initialized() else 1)
        self.embed_dp_size = max(1, self.world_size // self.embed_tp_size)
        self.attn_dp_size = max(1, self.world_size // self.attn_tp_size)
        self.lmhead_dp_size = max(1, self.world_size // self.lmhead_tp_size)
        self.comm_dict: Dict[str, Optional[Any]] = {"default_pg": get_default_group()}
        ...
        if dist.is_available() and dist.is_initialized() and self.world_size > 1:
            ...
            if self.lmhead_tp_size == self.embed_tp_size:
                ...
                lmhead_tp_group = init_comm_group(
                    global_rank=global_rank,
                    group_num=self.lmhead_dp_size,
                    world_size=self.world_size,
                    group_stride=1,
                    group_name="lmhead_tp_group",
                )

            self.comm_dict.update(
                {
                    "attn_tp_group": attn_tp_group,
                    "embed_tp_group": embed_tp_group,
    ...
    def tp_rank(self, tp_group_key: str) -> int:
        group = self.comm_dict.get(tp_group_key)
        if group is None or not dist.is_available() or not dist.is_initialized():
            return 0
        return dist.get_rank(group)


def build_row_parallel_linear(
    input_size: int,
    output_size: int,
    *,
    bias: bool = False,
...
def build_vocab_parallel_embedding(
    vocab_size: int,
    hidden_size: int,
    padding_idx: int,
    parallel_context: Qwen2_5_VLParallelContext,
    *,
    dtype: torch.dtype = torch.bfloat16,
):
    if parallel_context.embed_tp_size > 1:
        return VocabParallelEmbedding(
            vocab_size=vocab_size,
            hidden_size=hidden_size,
            padding_idx=padding_idx,
            params_dtype=dtype,
            tp_size=parallel_context.embed_tp_size,
            tp_rank=parallel_context.tp_rank("embed_tp_group"),
        )
...
class Qwen2_5_VLMLP(nn.Module):
    def __init__(
        ...
        *,
        ...
        tp_rank = parallel_context.tp_rank("attn_tp_group")
        tp_size = parallel_context.attn_tp_size
        self.gate_proj = build_column_parallel_linear(
            self.hidden_size,
        ...
        self.up_proj = build_column_parallel_linear(
            self.hidden_size,
            self.intermediate_size,
            bias=bias,
            tp_size=tp_size,
        ...
        self.down_proj = build_row_parallel_linear(
            self.intermediate_size,
            self.hidden_size,
            bias=bias,
            tp_size=tp_size,
    ...
    def forward(self, hidden_states, *args):
        if len(args) == 0:
            ...
            raise NotImplementedError(
                f"Unsupported Qwen2RMSNorm input signature length {len(args) + 1}."
            )
    ...
    **kwargs,
    ...
    if attention_mask is not None:
        ...
        if attn_mask.dim() == 4 and attn_mask.size(1) == 1:
            attn_mask = attn_mask.expand(-1, query.size(1), -1, -1)
    ...
    attn_output = F.scaled_dot_product_attention(
        ...
        is_causal=module.is_causal and attn_mask is None,
    )
    return attn_output, None


register_attention_backend("eager", eager_attention_forward)
register_attention_backend("sdpa", sdpa_attention_forward)
register_attention_backend("flash_attention_2", sdpa_attention_forward)
...
@auto_docstring
class Qwen2_5_VLPreTrainedModel(nn.Module):
    ...
    def __init__(self, config: Any, *inputs, **kwargs) -> None:
        super().__init__()
        self.config = config

    def post_init(self) -> None:
        """Hook for subclasses to run any final initialisation logic."""
        return
...
class Qwen2_5_VLAttention(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        tp_size = parallel_context.attn_tp_size
        tp_rank = parallel_context.tp_rank("attn_tp_group")
        self.q_proj = build_column_parallel_linear(
            self.hidden_size,
        ...
        self.k_proj = build_column_parallel_linear(
            self.hidden_size,
        ...
        self.v_proj = build_column_parallel_linear(
            self.hidden_size,
            self.num_key_value_heads * self.head_dim,
            bias=True,
            tp_size=tp_size,
        ...
        self.o_proj = build_row_parallel_linear(
            self.num_heads * self.head_dim,
            self.hidden_size,
            bias=False,
            tp_size=tp_size,
...
class Qwen2_5_VLTextModel(Qwen2_5_VLPreTrainedModel):
    ...
    def __init__(
        self,
        config: Qwen2_5_VLTextConfig,
        parallel_context: Optional[Qwen2_5_VLParallelContext] = None,
        *,
        prefix: str = "",
    ):
...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(config)
        self.runner_settings = runner_settings or {}
        self.parallel_context = Qwen2_5_VLParallelContext(self.runner_settings)
        self.model = Qwen2_5_VLModel(
            config,
            self.parallel_context,
            prefix=f"{prefix}.model",
        )
        self.lm_head = build_column_parallel_linear(
            input_size=config.text_config.hidden_size,
            output_size=config.text_config.vocab_size,
            bias=False,
            tp_size=self.parallel_context.lmhead_tp_size,
            tp_rank=self.parallel_context.tp_rank("lmhead_tp_group"),
    ...
    def forward(
        ...
        **kwargs,
        ...
        if (
            self.parallel_context.lmhead_tp_size > 1
            and dist.is_available()
            and dist.is_initialized()
        ):
            gathered = [logits.new_empty(logits.shape) for _ in range(self.parallel_context.lmhead_tp_size)]
            dist.all_gather(
                gathered,
                logits,
                group=self.parallel_context.comm_dict.get("lmhead_tp_group"),
            )
